...
def main():
	...
	filename = 'concatenated_50.csv'

	#read in the csv file and prepare data for classification
	x_train,x_test,y_train,y_test = read_and_preprocess_data_one_hot_encoder(filename)
	print x_train.shape
	...
	architectures = define_architectures(window_length,input_shape,stride)
...
def classification(model,x_train,y_train):
	...
	for train_index, test_index in kfold.split(x_train, converted_labels):
		...
		y_val = [y_train[index] for index in test_index]
		#train on the training set
		model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
		model.fit(x,y,batch_size = 8,verbose=1,epochs=100,validation_data = (x_val,y_val))
...
def define_architectures(window_length,input_shape,stride):
	#arbitrarily (Sort of ) defined conv architecutres
	model1 = Sequential()
	model1.add(Conv1D(input_shape = input_shape,filters=2,kernel_size=window_length,strides=stride))
	model1.add(Dropout(.2))
	model1.add(Flatten())
	model1.add(Dense(2,activation = 'sigmoid'))

	model2 = Sequential()
	model2.add(Conv1D(input_shape = input_shape,filters=32,kernel_size=window_length,activation='relu',strides=stride))
	...
	model3 = Sequential()
	model3.add(Conv1D(input_shape = input_shape,filters=16,kernel_size=window_length,strides=stride))
	...
	model4 = Sequential()
	model4.add(Conv1D(input_shape = input_shape,filters=32,kernel_size=window_length,strides=stride))
...
def read_and_preprocess_data_one_hot_encoder(filename):
	data = pd.read_csv(filename)
	temp =  data['0'].apply(lambda x: pd.Series(list(x)))
	temp = pd.get_dummies(temp)
	data = pd.concat([data,temp],axis=1)
	print data.shape
	### oversample 1s
	data = pd.concat([data,data[data['Labels'] == 1]])
	data = pd.concat([data,data[data['Labels'] == 1].iloc[0:162]])
	...
	features_transformed = data[[col for col in data.columns if '_' in col]].values
...
def read_and_preprocess_data_label_encoder(filename):
	data = pd.read_csv(filename)
	...
	for sequence in data['0'].values:
		features.append([nucleotide.lower().strip(' ') for nucleotide in sequence])
